...
// Read accelerometer function for 
int readAcc() { 
  ...
  //dataX[0] = dataHighres;  
  acc_min[0] = dataHighres; 
  acc_max[0]  = dataHighres;
  if( myIMU.readRegisterInt16( &dataHighres, KXTJ3_OUT_Y_L ) == 0 ){}
  //dataY[0] = dataHighres;
  acc_min[1] = dataHighres; 
  acc_max[1]  = dataHighres;
  if( myIMU.readRegisterInt16( &dataHighres, KXTJ3_OUT_Z_L ) == 0 ){}
  //dataZ[0] = dataHighres;
  acc_min[2] = dataHighres; 
  acc_max[2]  = dataHighres;
...
for (int i=1;i<samples;i++){
  ...
  if( myIMU.readRegisterInt16( &dataHighres, KXTJ3_OUT_X_L ) == 0 ){}
  //dataX[i] = dataHighres;
  if (acc_min[0] > dataHighres){
    acc_min[0] = dataHighres;    
    }
   if (acc_max[0] < dataHighres){
    acc_max[0] = dataHighres;    
    }
  if( myIMU.readRegisterInt16( &dataHighres, KXTJ3_OUT_Y_L ) == 0 ){}
  //dataY[i] = dataHighres;
  if (acc_min[1] > dataHighres){
    acc_min[1] = dataHighres;    
    }
   if (acc_max[1] < dataHighres){
    acc_max[1] = dataHighres;    
    }
  if( myIMU.readRegisterInt16( &dataHighres, KXTJ3_OUT_Z_L ) == 0 ){}
  //dataZ[i] = dataHighres;
  if (acc_min[2] > dataHighres){
    acc_min[2] = dataHighres;    
    }
   if (acc_max[2] < dataHighres){
    acc_max[2] = dataHighres;    
    }
    delay(10);
}  
...
acc_diff = acc_max[0]-acc_min[0];
acc_diff =acc_diff + acc_max[1]-acc_min[1];
acc_diff =acc_diff + acc_max[2]-acc_min[2];

if (acc_diff > 500) {  moov = true;  }
else {moov = false;}
}
...
void do_send(osjob_t* j){
    ...
    readAcc(); // read accelemeter and compute acc_diff and moov  
